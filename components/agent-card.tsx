...
const AgentCard: React.FC<AgentCardProps> = ({
  ...
  const displayTags = deprecated
    ? tags.filter((tag) => tag !== 'Live')
    : tags;

  return (
    <div
      className={`flex-1 rounded-lg bg-aipgf-white border-aipgf-geyser border-[1px] border-solid box-border overflow-hidden flex flex-col items-start justify-start gap-[0.9rem] min-w-[19.25rem] max-w-full relative ${
    ...
    >
      ...
      <div className="self-stretch flex flex-col items-start justify-start gap-[1.262rem] max-w-full text-left text-[0.869rem] text-aipgf-shark1 font-aipgf-manrope-semibold-1356">
        <div className="self-stretch flex flex-row flex-wrap items-center justify-start py-[0rem] pl-[1.312rem] pr-[1.375rem] box-border max-w-full gap-[0.5rem] text-[1.625rem]">
          <h1 className="m-0 relative text-inherit tracking-[-0.02em] leading-[2rem] font-bold font-[inherit] sm:text-[1.313rem] sm:leading-[1.625rem]">
            {name}
          </h1>
          {deprecated && (
            <span className="rounded-xl bg-aipgf-nevada px-2 py-1 text-xs font-semibold text-aipgf-white">
              Deprecated
            </span>
          )}
        </div>
        ...
        <div className="self-stretch h-[5.863rem] relative">
          <div className="absolute top-[1.475rem] left-[0rem] rounded-t-none rounded-b-3xs w-full flex flex-row flex-wrap items-start justify-start py-[1.35rem] pl-[1.312rem] pr-[13.062rem] box-border gap-[0.5rem] min-h-[2.813rem] z-[2] text-center text-[0.75rem] sm:pr-[1.25rem] sm:box-border">
            {displayTags.map((tag, index) => (
              <div
                key={index}
                className="flex-1 rounded-lg bg-aipgf-iron-50 flex flex-row items-center justify-start py-[0.093rem] px-[0rem] min-w-[4rem]"
              >
